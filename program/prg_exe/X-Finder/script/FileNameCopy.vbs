...
For lIdx = LBound( asTrgtNames ) to UBound( asTrgtNames )
    If lIdx = LBound( asTrgtNames ) Then
        sTrgtNames = asTrgtNames( lIdx )
    Else
        sTrgtNames = sTrgtNames & vbNewLine & asTrgtNames( lIdx )
    End If
Next

'*** クリップボードへコピー ***
CreateObject( "WScript.Shell" ).Exec( "clip" ).StdIn.Write( sTrgtNames )
