"http://bokut.in/mt/2007/02/alignvim.htmlhttp://bokut.in/mt/2007/02/alignvim.html
...
" StrWidth: returns a string width {{{1
fun! s:StrWidth(str)
  let str = a:str
  ...
  if &encoding == "utf-8"
	" 半角カナを1byteに
	let str = substitute(str, "[\uFF61-\uFF9F]", "x", "g")
	" 半角カナとアスキー以外を2byteに
	let str = substitute(str, "[^\x20-\x7E\uFF61-\uFF9F]", "xx", "g")
  elseif &encoding == "euc-jp"
	" 半角カナを1byteに
	let str = substitute(str, "[\u8EA1-\u8EFE]", "x", "g")
  else
	" その他
  endif
...
"     call Decho("AlignCtrl<".s:AlignCtrl."> AlignQty=".s:AlignPatQty." AlignPat_".ipat."<".s:AlignPat_{ipat}.">")
  ...
  if (begline == line("'>") && endline == line("'<")) || (begline == line("'<") && endline == line("'>"))
   ...
   if vmode == "\<c-v>"
    let ragged   = ( col("'>") > s:StrWidth(getline("'>")) || col("'<") > s:StrWidth(getline("'<")) )
   else
	let ragged= 1
   endif
  else
...
"  call Dfunc("AlignReplaceQuotedSpaces()")
  ...
  let l:linelen       = s:StrWidth(l:line)
  let l:startingPos   = 0
